...
- [x] Traffic sprite render: `tools/blender/render_traffic.py` (15 sideways offsets, ±12 m, 10 m ahead of the camera, 1:1 on a 1080-pixel screen; no occupants). Each frame shows the vehicle parallel to the road off to one side, with the camera looking straight ahead and lens shift centring it. Frames go by the offset in metres, not the viewing angle: the slope of the sides towards the vanishing point depends only on the offset, so they stay along the road at every distance. The game picks the frame from the slope of the vehicle's lane on screen (eye height × pixels across per pixel down), so in bends and on hills traffic points the way its lane runs. (Turning the vehicle made traffic point away from the road; picking by angle made it point inwards when passed.) `prep_car.py` is used as it is, with the length per vehicle. The camera, frame fitting and sheet packing are shared with the player car in `sprite_common.py`. Output goes to `local-assets/traffic/<name>/`.
- [ ] Stand-in traffic models are in (`deco-truck` 6.0 m, `kei-truck` 3.4 m, `supercar` 4.5 m, `yankee` 4.4 m, `bus` 7.3 m, `grey-sedan` 4.7 m, `kei-wagon` 3.4 m, `peanut-hatchback` 3.7 m, `station-wagon` 4.6 m, `yellow-sport` 4.3 m — the lengths given to `prep_car.py`). Before any of them ships:
  ...
  - [ ] `station-wagon` and `yellow-sport`: made-up lettering on the plates and a badge on the wagon's tailgate; check they read as nothing real. `yellow-sport` is close to a real Japanese rotary coupé; replace it with an original design.
  - [ ] `grey-sedan`: the rear window has no glass and the plate recess is a hole into the body. Fix the model or replace it.
...
- [ ] Traffic sheets are about 8 thousand pixels wide each, in two rows (three for the bus and the decorated truck). They count towards the texture budget (M4) and the 4096-pixel limit.
